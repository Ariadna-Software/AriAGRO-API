...
module.exports.getLogin = function(login, password, callback) {
    var usuario = null;
    var sql = "SELECT us.*, ari.telsoci1, ari.telsoci2, ari.telsoci3, ari.movsocio";
    sql += " FROM usuariospush AS us";
    sql += " LEFT JOIN " + cfg.ariagro.database + ".rsocios AS ari ON ari.codsocio = us.ariagroId"
...
module.exports.putUsuarioPush = function(id, usuarioPush, callback) {
    delete usuarioPush.telsoci1;
    delete usuarioPush.telsoci2;
    delete usuarioPush.telsoci3;
    delete usuarioPush.movsocio;
    if (!comprobarUsuarioPush(usuarioPush)) {
        var err = new Error("El usuarioPush pasado es incorrecto, no es un objeto de este tipo o le falta algún atributo olbligatorio");
        callback(err);
        return;
