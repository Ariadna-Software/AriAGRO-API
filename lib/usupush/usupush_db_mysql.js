...
// loginUsuariosPush
// busca un usuarioPush con el login y contraseña pasados
// si lo encuentra lo devuelve, si no devuelve nulo.
module.exports.loginUsuariosPush = function(usuarioPush, callback){
	var connection = conector.getConnectionPush();
	if (usuarioPush && usuarioPush.login && usuarioPush.password){
		var sql = "SELECT * FROM usuariosPush WHERE login = ? AND password = ? ORDER BY nombre";
		sql = mysql.format(sql, [usuarioPush.login, usuarioPush.password]);
		connection.query(sql, function(err, result){
			conector.closeConnection(connection);
			if (err){
				return callback(err, null);
		...
		var err = new Error('API: No se ha proporcionado un objeto usuarioPush con login y contraseña');
		return callback(err, null);
	}
	return;
}
