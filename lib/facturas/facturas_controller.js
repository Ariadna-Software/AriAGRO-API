...
router.get('/varias/:codclien/:year/:codsocio/:campanya', function(req, res) {
    var codclien = req.params.codclien;
    var year = req.params.year;
    var codsocio = req.params.codsocio;
    var campanya = req.params.campanya;
    if (codclien && year) {
        facturasVariasMysql.getFacturasVariasCliente(codclien, year, codsocio, campanya, function(err, facturas) {
            if (err) {
                return res.status(500).send(err.message);
            }
            return res.json(facturas)
