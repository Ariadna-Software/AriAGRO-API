...
module.exports.getFacturasTiendaCliente = function(codclien, year, callback) {
    ...
    // Ahora vamos a mostrar las facturas de tratamiento de manera diferenciada
    // Comprobamos de qué cooperativa se trata.
    if (config.database_gessocial != ""){
        // ES ALZICOOP, no hay que coger las de un determinado codtipom
        // que cogemos del parámetro "tipo_adv_tienda" del mysql_config
        var inSQL = config.tipo_adv_tienda.toString();
        var sql2 = " AND f.codtipom NOT IN (?)";
    ...
    if (config.tipo_fac_tienda.length > 0){
        var inSQL = config.tipo_fac_tienda.toString();
