...
module.exports.getFacturasTelefoniaCliente = function(codclien, year, callback) {
    ...
    var array = [];
    if(process.env.telefoniatipo_fac_telefonia){
        array = JSON.parse(process.env.telefoniatipo_fac_telefonia);//convertimos el objeto en json
    }
    if (process.env.telefoniatipo_fac_telefonia && array.length > 0){
