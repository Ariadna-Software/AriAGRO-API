...
module.exports.getFacturasTratamientosCliente = function(codclien, year, codsocio, campanya, callback) {
    ...
    var sql = "";
    var connection = null;
    if (process.env.tratamientosdatabase == "ariagro") {
        ...
        sql += " LEFT JOIN " + process.env.usuariosdatabase + ".stipom AS t ON (t.codtipom = f.codtipom)";
        ...
        var array = JSON.parse(process.env.tratamientostipo_fac_adv);//convertimos el objeto en json
        // ALZICOOP
        sql = "SELECT";
        sql += " f.codtipom AS codtipom,";
        sql += " f.numfactu AS numfactu,";
