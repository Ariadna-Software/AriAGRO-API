...
var crearJsonGasol = function(codclien, year, numfactu, letraser, callback) {
    ...
    var sql = "SELECT fc.codsocio, so.nomsocio, so.domsocio, so.codposta, so.pobsocio, so.prosocio, so.nifsocio, fc.letraser, fc.numfactu, fc.fecfactu,";
    ...
    sql += " fc.impuesto, fc.totalfac, fl.numlinea, fl.numalbar, fl.fecalbar, fl.horalbar, a.nomartic, fl.cantidad, fl.preciove, fl.implinea ";
    sql += " FROM schfac AS fc";
    sql += " LEFT JOIN slhfac AS fl ON fl.letraser = fc.letraser AND fl.numfactu = fc.numfactu AND fl.fecfactu = fc.fecfactu";
    sql += " LEFT JOIN sartic AS a ON a.codartic = fl.codartic";
