// enlaces_db_mysql
// Manejo de la tabla enlaces en la base de datos
...
// getEnlaces
// lee todos los registros de la tabla enlaces y
...
// getEnlacesBuscar
// lee todos los registros de la tabla enlaces cuyo
// nombre contiene la cadena de búsqueda. Si la cadena es '*'
// devuelve todos los registros
module.exports.getEnlacesBuscar = function (nombre, callback) {
    var connection = conector.getConnectionPush();
    var enlaces = null;
    var sql = "SELECT * FROM enlaces";
    if (nombre !== "*") {
        sql = "SELECT * FROM enlaces WHERE nombre LIKE ?";
...
module.exports.getEnlace = function(id, callback){
	var connection = conector.getConnectionPush();
	var enlaces = null;
	sql = "SELECT * FROM enlaces WHERE enlaceId = ?";
...
module.exports.putEnlace = function(id, enlace, callback){
	...
	var connection = conector.getConnectionPush();
	sql = "UPDATE enlaces SET ? WHERE enlaceId = ?";
...
module.exports.deleteEnlace = function(id, enlace, callback){
	...
	connection.query(sql, function(err, result){
		...
		enlace = result[0];
		sql = "DELETE from enlaces WHERE enlaceId = ?";
		sql = mysql.format(sql, id);
		connection.query(sql, function(err, result){
			conector.closeConnection(connection);
			if (err){
