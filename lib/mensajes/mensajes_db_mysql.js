...
module.exports.postPrepararCorreos = function (numalbar, done) {
    crearPdfsClasif(numalbar, (err, result) => {
        if (err) return done(err);
        done(null, result);
    });
}

module.exports.postEnviarCorreos = function (numalbar, email, ruta ,done) {
    // TODO: Hay que montar los correos propiamente dichos
    crearCorreosAEnviar(numalbar, email, ruta,(err, data) => {
        if (err) return done(err);
        var msg = data;
        done(null, msg);
    })
...
var crearPdfsClasif = function (numalbar, callback) {
    ...
    con.query(sql, function (err, result) {
        ...
        clasif = result;
            ...
            // var file = config2.reports_dir + "\\clasificacion.mrt";
            var file =__dirname + "\\public\\reports" + "\\clasificacion.mrt";
            ...
            report.renderAsync(function () {
                ...
                // fs.writeFileSync(config2.clasif_dir + "\\" + numalbar + ".pdf", buffer);
                // var ruta = config2.clasif_dir + "\\" + numalbar + ".pdf";
                fs.writeFileSync(__dirname + "\\public\\ficheros\\temporales\\" + numalbar + ".pdf", buffer);
                var ruta = __dirname + "\\public\\ficheros\\temporales\\" + numalbar + ".pdf";
                callback(null, ruta);
...
// crearCorreosAEnviar
var crearCorreosAEnviar = (numalbar, email, ruta, callback) => {
    // 1- creamos un correo con un asunto por defecto y sin texto
    var correo = {};
    correo.asunto = "Esta es la clasificacion solicitada del albaran "+ numalbar;
    correo.texto = "     ";
    ...
    transporter.sendMail(mailOptions, function(err, info){
        ...
        fs.unlink(__dirname + "\\public\\ficheros\\temporales\\" + numalbar + ".pdf", (err) => {
            return callback(err);
