...
module.exports.postPrepararCorreos = function (numalbar, campanya, informe, done) {
    crearJson(numalbar, campanya, informe, (err, result) => {
        if (err) return done(err);
        if (result) {
            crearPdfsClasif(numalbar, campanya, informe, (err, result) => {
                if (err) return done(err);
                done(null, result);
            });
        }
...
var crearJson = function(numalbar, campanya, informe, callback) {
    ...
    sql += " WHERE c.fecbajas IS NULL AND h.numalbar = 863626";
    con.query(sql, function (err, cab) {
        con.end();
        if (err) return callback(err, null);
        obj.cabecera_agro = cab;
        var con2 = conector.getConnectionCampanya(campanya);
        var sql2 = "SELECT ent.numalbar, ent.tiporecol,ent.numnotac, DATE_FORMAT(ent.fechaent,'%d/%m/%Y') AS fechaent, ent.kilosnet, ent.numcajon, ri.tipoentr";
        sql2 += " FROM rhisfruta_entradas AS ent"; 
        sql2 += " LEFT JOIN rhisfruta AS ri ON ri.numalbar = ent.numalbar";
        sql2 += " WHERE ent.numalbar = 863626";
        con2.query(sql2, function (err, entrada) {
            ...
            sql3 += " WHERE rhisfruta_clasif.kilosnet > 0 AND  rhisfruta_clasif.numalbar = 863626"
            con3.query(sql3, function (err, clasif) {
                ...
                sql4 += " WHERE inc.numalbar = 863626";
                con4.query(sql4, function (err, incidencia) {
                    con4.end();
                    if (err) return callback(err, null);
                    obj.incidencia_agro = incidencia;
                    var resultado = JSON.stringify(obj);
                    fs.writeFile("C:\\proyectos\\AriAGRO-API\\public\\json\\clasificacion.json", resultado, function(err) {
                        if(err) return done(err);
...
var crearPdfsClasif = function (numalbar, campanya, informe, callback) {
    Stimulsoft.Base.StiLicense.key = "6vJhGtLLLz2GNviWmUTrhSqnOItdDwjBylQzQcAOiHltN9ZO4D78QwpEoh6+UpBm5mrGyhSAIsuWoljPQdUv6R6vgv" +
        ...
        "oWT057ScSvGgQmfx8wCqSF+PgK/zTzjy75Oh";
            ...
            var file = config2.reports_dir + "\\" + "clasificacion-json.mrt";
