...
module.exports.postEnviarCorreos = function (numalbar, email, ruta , coop, done) {
    ...
    crearJson(numalbar, email, ruta, coop, (err, data) => {
        if (err) return done(err);
        var msg = data;
        done(null, msg);
    })
...
var crearJson = function(callback) {
...
var escribeGrupos = function(grupos, done) {
    var resultado = JSON.stringify(obj);
    fs.writeFile("C:\\proyectos\\gdes_pipeline\\public\\ficheros\\prueba.json", resultado, function(err) {
        if(err) return done(err);
    });
}
