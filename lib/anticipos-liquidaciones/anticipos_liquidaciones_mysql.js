...
var crearJsonLiquidacion = function(numfactu, campanya, codtipom, informe, servidor, callback) {
    ...
    sql += " f.codsocio, so.nomsocio, so.dirsocio, so.codpostal, so.pobsocio, so.prosocio, so.nifsocio, so.dirsociocorreo, so.pobsociocorreo, so.codpostalcorreo, f.codtipom, t.nomtipom, t.letraser, f.numfactu, DATE_FORMAT(f.fecfactu, '%d/%m/%y') AS fecfactu";
    sql += " FROM rfactsoc AS f";
    sql += " LEFT JOIN usuarios.stipom AS t ON t.codtipom = f.codtipom";
    ...
    con.query(sql, function (err, cab) {
        ...
        con2.query(sql2, function(err,lineas) {
            ...
            if (err) return callback(err, null);
            obj.lineas = lineas;
            var sql2Bis = "SELECT cal.codvarie, var.nomvarie,r.tipcalid,SUM(cal.imporcal) AS imporcal, SUM(kilosnet) AS kilosnet"
            ...
            con2Bis.query(sql2Bis, function(err,lineasBis) {
                ...
                con3.query(sql3, function(err,pie) {
                    ...
                    if (err) return callback(err, null);
                    obj.pie = pie;
                    var sql4 = "SELECT DATE_FORMAT(ant.fecfactu, '%d/%m/%y') AS fecfactu, ant.codtipom, ant.numfactu, ant.codtipomanti,";
                    sql4 += " ant.numfactuanti, DATE_FORMAT(ant.fecfactuanti, '%d/%m/%y') AS fecfactuanti, ant.codvarieanti, ant.codcampoanti,";
...
var crearJsonAnticipo = function(numfactu, campanya, codtipom, informe, servidor, callback) {
    ...
    con.query(sql, function (err, cab) {
        ...
        con2.query(sql2, function(err,lineas) {
            ...
            obj.lineas = lineas;
            //Castelduc
            var sql2Bis = "SELECT f.codvarie, SUM(f.imporvar) AS imporvar, SUM(f.kilogrado) AS kilogrado, SUM(f.kilosnet) AS kilosnet , f.preciomed, f.codcampo, var.nomvarie, par.nomparti, par.codparti"
            ...
            con2Bis.query(sql2Bis, function(err,lineasBis) {
                ...
                con3.query(sql3, function(err,pie) {
                    ...
                    if (err) return callback(err, null);
                    obj.pie = pie;
                    var sql4 = "SELECT var.codtipom, var.numfactu, DATE_FORMAT(var.fecfactu, '%d/%m/%y') AS fecfactu, var.codtipomfvar, RIGHT(CONCAT('0000000',var.numfactufvar),7) AS numfactufvarias, DATE_FORMAT(var.fecfactufvar, '%d/%m/%y') AS fecfactufvar,";
