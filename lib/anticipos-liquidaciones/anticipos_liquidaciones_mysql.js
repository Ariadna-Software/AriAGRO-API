        ...
        con2.query(sql2, function(err,lineas) {
            ...
            con2Bis.query(sql2Bis, function(err,lineasBis) {
                ...
                con3.query(sql3, function(err,pie) {
                    ...
                    sql4 += " var.numfactu, p.totalfac AS tot, COALESCE(p.totalfac-v.totalvarias, 0) AS result,";
                    sql4 += " var.fecfactu AS fecfactu, ";
                    sql4 += " var.codtipomfvar, RIGHT(CONCAT('0000000',var.numfactufvar),7) AS numfactufvarias, ";
                    sql4 += " var.fecfactufvar AS fecfactufvar,";
                    sql4 += " var.codsecci, f.totalfac";
                    sql4+= " FROM rfactsoc_fvarias AS var ";
                    sql4 += " LEFT JOIN fvarcabfact  AS f ON f.numfactu = var.numfactufvar  AND f.fecfactu = var.fecfactufvar AND f.codtipom = var.codtipomfvar";
                    sql4 += " LEFT JOIN  (SELECT numfactu, fecfactu, codtipom, SUM(totalfac) AS totalVarias FROM fvarcabfact ) AS v ON  v.numfactu = var.numfactufvar AND v.fecfactu = var.fecfactufvar AND v.codtipom = var.codtipomfvar";
                    sql4 += " LEFT JOIN (SELECT numfactu, fecfactu, codtipom, totalfac FROM rfactsoc) AS p ON  p.numfactu = var.numfactu AND p.fecfactu = var.fecfactu AND p.codtipom = var.codtipom";
                    ...
                    con4.query(sql4, function(err, varias) {
                        ...
                        if(varias.length > 0) {
                            for(var i = 0; i < varias.length; i++) {
                                varias[i].fecfactu = moment(anticipos[i].fecfactu).format('DD/MM/YYYY');
                                varias[i].fecfactuvar = moment(anticipos[i].fecfactuvar).format('DD/MM/YYYY');
                            }
