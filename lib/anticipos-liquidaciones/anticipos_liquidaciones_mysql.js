...
module.exports.postEnviarCorreos = function (numfactu, email, ruta , coop, done) {
    // TODO: Hay que montar los correos propiamente dichos
    crearCorreosAEnviar(numfactu, email, ruta, coop, (err, data) => {
        if (err) return done(err);
        var msg = data;
        done(null, msg);
    })
...
// crearCorreosAEnviar
var crearCorreosAEnviar = (numfactu, email, ruta, coop, callback) => {
    // 1- creamos un correo con un asunto por defecto y sin texto
    var correo = {};
    correo.asunto = '['+coop+'] Clasificación'
    correo.texto = "Estimado socio/socia esta es la factura numero  "+ numfactu +" solicitada.\n Reciba un cordial saludo";
