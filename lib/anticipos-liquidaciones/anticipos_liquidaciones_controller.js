...
router.post('/preparar-correo', function (req, res) {
    ...
    anticiposLiquidacionesMysql.postPrepararCorreos(numfactu, campanya, informe, codtipom, servidor, function (err, result) {
 ...
 router.post('/enviar/correo/ant-liq', function (req, res) {
    ...
    anticiposLiquidacionesMysql.postEnviarCorreos(numfactu, email, ruta, coop, function (err, result) {
         if (err) return res.status(500).send(err.message);
         res.json(result);
     });
 });
